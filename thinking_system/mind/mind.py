...
class Mind:
    ...
    def attempt(self, task_id: str, train_pairs: list, *, effort: int = 2) -> dict:
        """Искать программу по train-парам; бюджет растёт по лестнице до effort."""
    ...
    def program_for(self, task_id: str) -> Program | None:
        ...
        if names is None:
            return None
        by = {p.name: p for p in self.prims}
        return Program([by[n] for n in names])
    ...
    def _add_abstraction(self, step_names: list[str]) -> bool:
        """Композиция базовых имён → новый примитив «a∘b» (если исполнима и нова)."""
        name = "∘".join(step_names)
        existing = {p.name for p in self.prims}
        if name in existing or any(n not in existing for n in step_names):
            return False
        by = {p.name: p for p in self.prims}
        prog = Program([by[n] for n in step_names])
    ...
    def _weights(self) -> dict[str, float] | None:
        ...
        return {p.name: c.get(p.name, 0) + 1.0 for p in self.prims}
