...
ГРАНИЦЫ: формы должны совпадать ПОБУКВЕННО (морфологии нет): «Сократ это человек» и
запрос про «человека» не свяжутся — «человек» ≠ «человеком». Без «это» утверждение
разбирается грубо как (первый токен, последний токен), а «все B P» берёт (rest[0],
rest[-1]), теряя слова в середине. Дедуктивная база под этим корректна; хрупок именно
разбор языка. Вне шаблонов выше результат не гарантирован.
"""
