...
ГРАНИЦЫ: «понимает новые предложения» = подставляет известные токены в позиционную
композицию. Грамматики нет: порядок слов ДОЛЖЕН совпадать с порядком исполнения
(«сумма удвоенного» не разберётся), синоним/другой падёж незнакомы, аргументы — только
целые/списки целых. Это не семантический разбор, а сопоставление по словарю.
"""
