...
ГРАНИЦЫ: разбор требует ЛИТЕРАЛЬНЫХ триггеров — слова «это» и одного из {сначала,
потом, затем, после}. «X = A затем B» или «X означает A потом B» не распознаются
(None). Это фиксированный шаблон определения, а не свободный разбор.
"""
