...
Небольшой обучаемый классификатор (мешок слов + линейный softmax) сопоставляет
команду одной из пространственных целей (углы + центр). Обучается на одних
КОМБИНАЦИЯХ глагол×формулировка×детерминант, проверяется на ДРУГИХ комбинациях.

Честно про границы: словарь ЗАКРЫТЫЙ (≈30 слов из PLACES/VERBS). «Невиданные
формулировки» — это невиданные КОМБИНАЦИИ уже знакомых слов, а не новые слова:
каждое слово теста встречалось в обучении (BagOfWords молча отбрасывает OOV).
Поэтому 100% на held-out = надёжная рекомбинация в закрытом словаре, а НЕ понимание
смысла слов и НЕ обобщение на синонимы вне словаря (напр. «northwestern» не сработает).
"""
