#!/usr/bin/env python3
"""Рост библиотеки из найденных поиском решений: система сама именует частые комбо.

Демонстрация механизма на СИНТЕТИЧЕСКИХ ARC-подобных задачах (НЕ на корпусе ARC —
датасета ARC в репозитории нет; это иллюстрация приёма, а не замер на бенчмарке).
На глубине 2 решатель находит программы, и ДВА комбо повторяются в разных задачах:
    «keep_largest ▸ bbox»  — выделить крупнейший объект и обрезать до него
    «flip_h ▸ flip_v»      — поворот на 180°
grow_from_solutions берёт эти найденные решения (поиском best_first_induce, а не
...
# синтетические ARC-подобные задачи: крупный объект + шум; и пары на поворот 180°
...
def main():
    print("▶ Рост библиотеки из найденных поиском решений (ARC-подобные задачи): имена для комбо\n")
    ...
    # 3) НОВЫЕ ядра, ТО ЖЕ преобразование: были недостижимы на глубине 1, теперь берутся.
    #    (это показывает сокращение глубины поиска от абстракции, а не «открытие» неизвестного:
    #     выход построен той же целевой композицией, поэтому решаемость на глубине 1 ожидаема)
    print("\n   ЗАДАЧА (новое ядро)      | seed, глубина 1 | после роста, глубина 1")
    raw = Library(SEED)
    ...
    print("   назвала. Назвав найденное, она решает такой класс задач на глубине 1 вместо 2 —")
    print("   опыт решений сокращает будущий поиск. Это иллюстрация механизма абстракции на")
    print("   синтетических ARC-подобных задачах; замера на корпусе ARC здесь нет.")
