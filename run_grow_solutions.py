...
СИНТЕТИЧЕСКАЯ иллюстрация механизма (данные ARC и их загрузчик в репозиторий НЕ
входят). На 6 рукотворных сетках внешний решатель best_first_induce находит решения,
и два комбо повторяются:
    «keep_largest ▸ bbox»  — выделить крупнейший объект и обрезать до него
    «flip_h ▸ flip_v»      — поворот на 180°
grow_from_solutions берёт эти решения (найденные поиском, а не собственным wake) и
абстрагирует повторяющиеся комбо в именованные операции. После этого такие задачи
решаются на глубине 1. Тот же механизм применим к настоящему ARC, но количественный
результат на нём ЭТИМ скриптом не воспроизводится (нет данных/харнесса в репозитории).
...
def main():
    ...
    print("   назвала. Тот же механизм применим к ARC, где комбо вроде «keep_largest ▸ bbox»")
    print("   и «flip_h ▸ flip_v» повторяются; но это СИНТЕТИЧЕСКАЯ демонстрация — данные ARC")
    print("   в репозиторий не входят, число решений на реальном ARC здесь не замеряется.")
    print("   Назвав найденное, система решает такой класс задач на глубине 1 — опыт")
    print("   решений сокращает будущий поиск.")
