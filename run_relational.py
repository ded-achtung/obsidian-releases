#!/usr/bin/env python3
"""Реляционное рассуждение: из нескольких фактов — много выводов.

Знания как отношения + правила Хорна; прямой вывод доводит до замыкания. Без
корпуса: транзитивность, наследование свойств, дедукция родственных связей.

Плюс ПЕРЕНОС ОТНОШЕНИЯ A:B :: C:? — по паре A→B система определяет, КАКОЕ это
отношение, и применяет его к C. Честно: это не вывод нового факта, а перенос —
ответ для C должен уже быть известен в базе (это disambiguation отношения по
примеру + lookup, а не аналогическая индукция отсутствующих фактов).
...
def main():
    ...
    # 3) перенос отношения A:B :: C:?  (определить отношение по A→B и применить к C)
    an = KnowledgeBase()
    ...
    print("\n3) ПЕРЕНОС ОТНОШЕНИЯ A:B :: C:? (по A→B определяет отношение, применяет к C):")
    ...
    print("   (честно: ответ для C берётся из базы — это перенос известного отношения,")
    print("    а не вывод отсутствующего факта)")

    print("\n── Итог ──")
    print("   Из горстки отношений и правил прямой вывод даёт множество НОВЫХ фактов")
    print("   (транзитивность, наследование, родство) — без корпуса, дедукцией из приоров.")
    print("   Перенос отношения A:B::C:? — отдельная, более простая операция: определить")
    print("   отношение по примеру и применить к C по базе (не индукция новых фактов).")
