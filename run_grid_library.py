...
абстрагирует их в новые именованные операции. Комбо, которые она выделяет здесь
(«bbox ▸ fractal», «trim_border ▸ mirror_quad»), — это операции из ARC-подобного
семейства задач (синтетические сетки; корпуса ARC в репозитории нет, замера на нём
здесь не проводится). Назвав комбо, система решает такие задачи на МЕНЬШЕЙ глубине
(depth 2 → depth 1).
...
def main():
    ...
    # те же комбо на НОВЫХ ядрах — теперь решаются на глубине 1.
    # (выход построен той же целевой композицией, поэтому решаемость на глубине 1 ожидаема:
    #  демонстрируется сокращение глубины поиска от абстракции, а не «открытие» неизвестного)
    ...
    print("   назвала. Назвав их, она берёт целое семейство задач на глубине 1 вместо 2 —")
    print("   поиск сокращается по мере роста опыта. Это путь к общности: не больше ручных")
    print("   примитивов, а самонаращиваемый словарь поверх них (на синтетических")
    print("   ARC-подобных задачах; замера на корпусе ARC здесь нет).")
