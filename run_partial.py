...
def main():
    ...
    print(f"   {'агент':<22}{'дошёл до цели':>16}{'сред. шагов':>14}")
    print(f"   {'belief (интегрирует)':<22}{f'{100 * b_ok / n:.0f}%':>16}{b_steps / max(b_ok, 1):>14.0f}")
    print(f"   {'безпамятный (random)':<22}{f'{100 * m_ok / n:.0f}%':>16}{m_steps / n:>14.0f}")

    print("\n── Итог ──")
    print("   Под частичной наблюдаемостью агент интегрирует наблюдения во времени в")
    print("   веру о позиции, активно её уточняет и достигает цели — там, где реакция")
    print("   на одно наблюдение (безпамятный) почти не справляется.")
